...
public class ConfigBuilder {
    ...
    private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
        ...
        packageInfo.put(ConstVal.REQDTO, joinPackage(config.getParent(), config.getReqDto()));
        packageInfo.put(ConstVal.RESPONSEDTO, joinPackage(config.getParent(), config.getResponseDto()));
        ...
        if (null != configPathInfo) {
            ...
            setPathInfo(pathInfo, template.getReqDto(), outputDir, ConstVal.REQ_DTO_PATH, ConstVal.REQDTO);
            setPathInfo(pathInfo, template.getResponseDto(), outputDir, ConstVal.RESPONSE_DTO_PATH, ConstVal.RESPONSEDTO);
        }
    }
    ...
    private List<TableInfo> processTable(List<TableInfo> tableList, NamingStrategy strategy, StrategyConfig config) {
        ...
        for (TableInfo tableInfo : tableList) {
            ...
            if (StringUtils.isNotEmpty(globalConfig.getReqDtoName())) {
                tableInfo.setReqDtoName(String.format(globalConfig.getReqDtoName(), entityName));
            } else {
                tableInfo.setReqDtoName(entityName + ConstVal.REQDTO);
            }
            if (StringUtils.isNotEmpty(globalConfig.getResponseDtoName())) {
                tableInfo.setResponseDtoName(String.format(globalConfig.getResponseDtoName(), entityName));
            } else {
                tableInfo.setResponseDtoName(entityName + ConstVal.RESPONSEDTO);
            }
            // 检测导入包
            ...
            try (
                ...
                while (results.next()) {
                    ...
                    if (strategyConfig.includeSuperEntityColumns(field.getName())) {
                        // 跳过公共字段  自己修改公共字段也放入entity中；；；continue;
